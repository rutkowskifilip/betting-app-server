...
module.exports = {
  ...
  updateWinnersPoints: () => {
    ...
    db.query(
      `UPDATE winners_bets u JOIN (SELECT first, second FROM winners_bets WHERE userId = 0) temp ON 1=1 SET u.pointss = u.pointss + (CASE WHEN u.first = temp.first THEN ${winners} ELSE 0 END) + (CASE WHEN u.second = temp.second THEN ${winners} ELSE 0 END)  WHERE u.userId <> 0;`
    );
  },
};
